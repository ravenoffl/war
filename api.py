...
def configure_ngrok():
    ngrok_token = "2m2tioSr0q0HLkywmGy4Qxa7O4p_wbg23jdBdi2hk8qojX8X"
    try:
...
def update_soul_txt(public_url):
    with open("tata1.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in tata1.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("tata1.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated tata1.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update tata1.txt on VPS: {str(e)}")
